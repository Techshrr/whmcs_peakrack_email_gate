...
3. Clear the WHMCS template cache if the client verification page still shows active code inputs during lockout.
4. Test five incorrect code attempts and confirm the six code inputs become grey and cannot be edited during the lockout window.

No database migration is required from 1.1.7 to 1.1.8.
...
1. Back up the WHMCS database.
2. Replace `modules/addons/peakrack_email_gate/` with the new `peakrack_email_gate/` directory.
3. Open **Addons > PeakRack Email Verification Gate** once so the default email template can be normalized.
4. Send one test verification email and confirm the message body follows the WHMCS email container width.

No database migration is required from 1.1.5 to 1.1.6.

## Upgrade to 1.1.5

1. Back up the WHMCS database.
2. Replace `modules/addons/peakrack_email_gate/` with the new `peakrack_email_gate/` directory.
3. Open **Addons > PeakRack Email Verification Gate** once so the new button color setting and email defaults are normalized.
4. Review the English and Chinese email templates if you previously customized them.
5. Clear the WHMCS template cache if the client verification page still shows the old label layout.

No database migration is required from 1.1.4 to 1.1.5.

## Upgrade to 1.1.4

1. Back up the WHMCS database.
2. Replace `modules/addons/peakrack_email_gate/` with the new `peakrack_email_gate/` directory.
3. Clear the WHMCS template cache if the client verification page still shows old spacing or button text.
4. Test the checkout flow with an unverified client and confirm the fallback countdown sends the client to the gate page if WHMCS cannot redirect immediately.

No database migration is required from 1.1.3 to 1.1.4.
...
4. Request a verification email and confirm the resend button counts down before it can be clicked again.
